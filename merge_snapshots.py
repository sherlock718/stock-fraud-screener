...
Usage:
  python3 merge_snapshots.py               # dry run — show counts only
  python3 merge_snapshots.py --activate    # write combined files as the active dataset
  python3 merge_snapshots.py --activate --backup  # backup existing files first
...
After --activate, run:
  python3 run_pipeline.py build --step 4   # macro → features → clean on combined dataset
"""
