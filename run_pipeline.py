...
Usage:
  python run_pipeline.py build              # Full pipeline (all steps)
  python run_pipeline.py build --limit 200  # Test run (200 companies)
  python run_pipeline.py build --step 2     # Resume from step 2
  python run_pipeline.py features           # Recompute features only (step 5, no API)
  python run_pipeline.py enrich-prices      # Re-run price enrichment (step 3)
  python run_pipeline.py enrich-macro       # Re-run macro enrichment (step 4)
  python run_pipeline.py clean              # Re-run clean (step 6)
  python run_pipeline.py status             # Show dataset stats
...
def run_step(step_num, extra_args=None):
    ...
    if result.returncode != 0:
        ...
        log(f'  python run_pipeline.py build --step {step_num}')
        sys.exit(1)
