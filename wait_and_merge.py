...
Run this once in a terminal and leave it. It will:
  ...
  2. Run merge_snapshots.py --activate --backup
  3. Run python3 run_pipeline.py build --step 4  (features + clean on full dataset)

Usage:
  python3 wait_and_merge.py
"""
...
run(['python3', 'merge_snapshots.py', '--activate', '--backup'])

print('\n─── Step 2: Re-running features + clean on merged dataset ───')
run(['python3', 'run_pipeline.py', 'build', '--step', '4'])
