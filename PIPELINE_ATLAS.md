...
- **tests/pipeline/test_p0f.py** covers: structural rules (annual only, FY bounds, quarterly excluded), investable filters (revenue/assets/price floors, SIC exclusions), fail-open (NaN revenue/assets/SIC pass), fail-closed (NaN price excluded), market-specific price floors (US/CA/JP/BR/DE/unknown), in_universe column semantics (dtype, binary, pipe-separated reasons, no mutation), missing column handling. 48 tests.
- **Zero coverage** for: universe/confidence, fraud labels, multi-market logic.
