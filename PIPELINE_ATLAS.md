...
|---|---|---|---|---|---|---|---|
| `step1_fetch_tickers.py` | Ticker ingestion (US) | Duplicates, missing CIK, inactive tickers included, suffix errors | Schema validation, dedup, inactive handling | `tests/test_step1.py` | P1 | None | missing |
...
| `step2_build_snapshots.py` | EDGAR XBRL parse | Wrong units, duplicate rows, missing filed_date, quarterly leak | One-row-per-ticker-year, filed_date≥period_end | `tests/test_step2.py` | P0 | None | missing |
...
| `step3_enrich_prices.py` | Price + forward returns | Look-ahead bias (entry price before filed_date), survivorship | entry_date > filed_date, forward returns future-only, delisted handling | `tests/test_step3.py` | P0 | None | missing |
...
- **Zero coverage** for: all 6 pipeline steps, feature_library, universe/confidence, fraud labels, any multi-market logic.
