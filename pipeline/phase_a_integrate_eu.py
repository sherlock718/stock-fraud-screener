"""
Phase A — Integrate Europe (EU) market data into historical_dataset_clean.parquet

EU snapshots (data/snapshots_eu.parquet) are built via yfinance free-tier (~4–5 years
of history for major European exchange tickers across DE, FR, NL, BE, PT, NO, FI, DK,
SE, IE, and other markets in tickers_eu.parquet).

This script:
  1. Loads data/snapshots_eu.parquet and standardises column names
  2. Runs step3 price enrichment for EU tickers -> data/prices_eu.parquet
  3. Merges EU snapshots + prices + macro
...
EU_SNAPSHOTS = DATA / 'snapshots_eu.parquet'
EU_PRICES    = DATA / 'prices_eu.parquet'
EU_STD       = DATA / 'snapshots_eu_std.parquet'
MACRO        = DATA / 'macro_eu.parquet'
...
def standardise_eu_snapshots(df: pd.DataFrame) -> pd.DataFrame:
    """Add missing columns and aliases so step5 functions can run on EU data."""
    df = df.copy()
    ...
    # total_liabilities: estimate if missing
...
def run_step3_for_eu(eu_std: pd.DataFrame) -> pd.DataFrame:
    eu_std.to_parquet(EU_STD, index=False)
    print(f'  Saved standardised EU snapshots: {EU_STD} ({len(eu_std):,} rows)')

    import pipeline.step3_enrich_prices as step3
    print('  Running step3 price enrichment for EU tickers...')
    step3.run(snapshots_path=str(EU_STD), out_path=str(EU_PRICES))

    if not EU_PRICES.exists():
        print('  WARN: step3 produced no output — prices_eu.parquet missing')
        return pd.DataFrame()

    prices = pd.read_parquet(EU_PRICES)
    print(f'  Loaded EU prices: {len(prices):,} rows × {len(prices.columns)} cols')
    return prices


def merge_eu_data(eu: pd.DataFrame, prices: pd.DataFrame) -> pd.DataFrame:
    macro_path = MACRO if MACRO.exists() else MACRO_FALLBACK

    if prices.empty:
        print('  WARN: No price data — EU rows will have NaN price/return columns')
        merged = eu.copy()
    else:
        price_cols = set(prices.columns)
        avail_keys = [k for k in MERGE_KEYS if k in eu.columns and k in price_cols]
        print(f'  Merging on keys: {avail_keys}')
        snap_cols = set(eu.columns)
        price_only_cols = [c for c in prices.columns if c not in snap_cols or c in avail_keys]
        merged = eu.merge(prices[price_only_cols], on=avail_keys, how='left', suffixes=('', '_price'))
...
def run(dry_run: bool = False) -> None:
    print('Phase A — Integrate Europe (EU) market data')
    print('=' * 60)

    if not EU_SNAPSHOTS.exists():
        print(f'ERROR: {EU_SNAPSHOTS} not found — run run_pipeline_eu.py build first')
        sys.exit(1)
    if not CLEAN.exists():
    ...
    if 'market' in existing.columns:
        eu_existing = existing['market'].isin(
            ['DE', 'FR', 'NL', 'BE', 'PT', 'NO', 'FI', 'DK', 'SE', 'IE', 'EU']
        ).sum()
        if eu_existing > 0:
            print(f'  NOTE: {eu_existing:,} EU rows already present — will be replaced')
            existing = existing[~existing['market'].isin(
                ['DE', 'FR', 'NL', 'BE', 'PT', 'NO', 'FI', 'DK', 'SE', 'IE', 'EU']
            )].copy()

    print('\n[2/9] Loading and standardising EU snapshots...')
    eu_raw = pd.read_parquet(EU_SNAPSHOTS)
    print(f'  EU raw: {len(eu_raw):,} rows × {len(eu_raw.columns)} cols')
    print(f'  Annual rows: {(eu_raw.get("period_type", pd.Series(dtype=str)) == "annual").sum():,}')
    if 'market' in eu_raw.columns:
        print(f'  Markets: {sorted(eu_raw["market"].dropna().unique().tolist())}')

    eu = standardise_eu_snapshots(eu_raw)
    print(f'  After standardisation: {len(eu.columns)} cols')

    print('\n[3/9] Running step3 price enrichment for EU...')
    try:
        prices = run_step3_for_eu(eu)
    except Exception as e:
        print(f'  WARN: step3 failed ({e}) — continuing without price data')
        prices = pd.DataFrame()

    print('\n[4/9] Merging snapshots + prices + macro...')
    df = merge_eu_data(eu, prices)
    ...
    print('\n[9/9] Aligning EU rows to target schema...')
    for col in target_cols:
        if col not in df.columns:
            df[col] = np.nan
    eu_aligned = df[target_cols].copy()
    print(f'  EU aligned: {len(eu_aligned):,} rows × {len(eu_aligned.columns)} cols')

    annual_eu = (eu_aligned['period_type'] == 'annual').sum() if 'period_type' in eu_aligned.columns else 0
    print(f'\n  EU rows summary:')
    print(f'    Total rows:  {len(eu_aligned):,}')
    print(f'    Annual rows: {annual_eu:,}')
    if 'market' in eu_aligned.columns:
        print(f'    Markets present: {sorted(eu_aligned["market"].dropna().unique().tolist())}')
    if 'entry_price' in eu_aligned.columns:
        print(f'    Rows with entry_price: {eu_aligned["entry_price"].notna().sum():,}')
    if 'beneish_m_score' in eu_aligned.columns:
        print(f'    Rows with beneish_m_score: {eu_aligned["beneish_m_score"].notna().sum():,}')
    ...
    print('\n  Concatenating...')
    combined = pd.concat([existing, eu_aligned], ignore_index=True)
    ...
    print(f'\n  Saved: {CLEAN}')
    print('Phase A — EU complete.')


def main() -> None:
    parser = argparse.ArgumentParser(description='Integrate EU market data (Phase A)')
