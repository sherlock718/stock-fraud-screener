"""
Step 2 JP Free — Build financial snapshots via yfinance (no EDINET key required).

Reads the curated static list from step1_fetch_tickers_jp_free.py (~130 major
Nikkei 225 + TSE Prime companies) and fetches 4-5 years of annual fundamentals
via yfinance.

For full TSE coverage (3,800+ companies with XBRL depth), use the EDINET-based
pipeline (step2_build_snapshots_jp.py) which requires a free EDINET_API_KEY.

Output: data/snapshots_jp.parquet — same schema as snapshots.parquet,
  market='JP', country='JP', accounting_std='IFRS', currency='JPY'
"""
...
def run(limit: int | None = None):
    ...
    if not TICK.exists():
        raise FileNotFoundError(f'{TICK} not found — run step1_fetch_tickers_jp_free.py first')
    ...
    print(f'Step 2 JP Free — Building snapshots via yfinance for {len(tickers):,} companies')
    print(f'  NOTE: yfinance provides ~4-5 years of annual data (JPY)')
    print(f'  For full TSE coverage with XBRL depth, register free EDINET key')
    ...
    for _, tkr_row in tickers.iterrows():
        ...
        exchange   = tkr_row.get('exchange', 'TSE')
    ...
    if not all_rows:
        print('ERROR: no data loaded — check tickers in tickers_jp.parquet')
    ...
    print(f'  Currency:         JPY (values in raw yen)')
    print(f'  Limitation:       yfinance ~4-5 years; full history needs EDINET key')
    print(f'  Saved: {OUT}')
