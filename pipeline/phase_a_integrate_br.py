"""
Phase A — Integrate Brazil (BR) market data into historical_dataset_clean.parquet

BR snapshots (data/snapshots_br.parquet) are built from CVM bulk CSV downloads
(IFRS filings, 2010-2025).

This script:
  1. Loads data/snapshots_br.parquet and standardises column names
  2. Runs step3 price enrichment for BR tickers -> data/prices_br.parquet
  3. Merges BR snapshots + prices + macro
...
BR_SNAPSHOTS = DATA / 'snapshots_br.parquet'
BR_PRICES    = DATA / 'prices_br.parquet'
BR_STD       = DATA / 'snapshots_br_std.parquet'
MACRO        = DATA / 'macro_br.parquet'
...
def standardise_br_snapshots(df: pd.DataFrame) -> pd.DataFrame:
    """Add missing columns so step5 functions can run on BR data."""
    df = df.copy()

    # BR uses 'equity' directly — step5 also expects 'equity'
    # BR uses 'receivables' directly — step5 also expects 'receivables'
    # BR uses 'operating_cash_flow' directly
    ...
    # SIC codes absent for BR — set NaN
    if 'sic_code' not in df.columns:
        df['sic_code'] = np.nan
    if 'sic_description' not in df.columns:
        df['sic_description'] = ''

    # total_liabilities: CVM doesn't provide directly — estimate from total_assets - equity
    ...
    # total_debt: CVM gives long_term_debt as full non-current liabilities; use as proxy
    if 'total_debt' not in df.columns:
        if 'long_term_debt' in df.columns:
            df['total_debt'] = df['long_term_debt'].fillna(0) + df.get('short_term_debt', pd.Series(0, index=df.index)).fillna(0)
        else:
            df['total_debt'] = np.nan

    # Columns expected by step5 but absent from BR snapshots
    for col in ['short_term_debt', 'retained_earnings', 'goodwill', 'intangibles',
                'other_noncurrent_assets', 'accounts_payable', 'ppe_net', 'ppe_gross',
                'non_operating_income', 'eps_basic', 'shares_outstanding',
                'rd_expense', 'sga_expense', 'ebitda', 'dividends_per_share',
                'dividends_paid', 'financing_cash_flow', 'stock_code', 'currency',
...
def run_step3_for_br(br_std: pd.DataFrame) -> pd.DataFrame:
    br_std.to_parquet(BR_STD, index=False)
    print(f'  Saved standardised BR snapshots: {BR_STD} ({len(br_std):,} rows)')

    import pipeline.step3_enrich_prices as step3
    print('  Running step3 price enrichment for BR tickers...')
    step3.run(snapshots_path=str(BR_STD), out_path=str(BR_PRICES))

    if not BR_PRICES.exists():
        print('  WARN: step3 produced no output — prices_br.parquet missing')
        return pd.DataFrame()

    prices = pd.read_parquet(BR_PRICES)
    print(f'  Loaded BR prices: {len(prices):,} rows × {len(prices.columns)} cols')
    return prices


def merge_br_data(br: pd.DataFrame, prices: pd.DataFrame) -> pd.DataFrame:
    macro_path = MACRO if MACRO.exists() else MACRO_FALLBACK

    if prices.empty:
        print('  WARN: No price data — BR rows will have NaN price/return columns')
        merged = br.copy()
    else:
        price_cols = set(prices.columns)
        avail_keys = [k for k in MERGE_KEYS if k in br.columns and k in price_cols]
        print(f'  Merging on keys: {avail_keys}')

        snap_cols = set(br.columns)
        price_only_cols = [c for c in prices.columns if c not in snap_cols or c in avail_keys]
        merged = br.merge(prices[price_only_cols], on=avail_keys, how='left', suffixes=('', '_price'))
...
def run(dry_run: bool = False) -> None:
    print('Phase A — Integrate Brazil (BR) market data')
    print('=' * 60)

    if not BR_SNAPSHOTS.exists():
        print(f'ERROR: {BR_SNAPSHOTS} not found — run run_pipeline_br.py build --step 1 first')
        sys.exit(1)
    if not CLEAN.exists():
    ...
    if 'market' in existing.columns:
        br_existing = (existing['market'] == 'BR').sum()
        if br_existing > 0:
            print(f'  NOTE: {br_existing:,} BR rows already present — will be replaced')
            existing = existing[existing['market'] != 'BR'].copy()

    print('\n[2/9] Loading and standardising BR snapshots...')
    br_raw = pd.read_parquet(BR_SNAPSHOTS)
    print(f'  BR raw: {len(br_raw):,} rows × {len(br_raw.columns)} cols')
    print(f'  Annual rows: {(br_raw.get("period_type", pd.Series(dtype=str)) == "annual").sum():,}')

    br = standardise_br_snapshots(br_raw)
    print(f'  After standardisation: {len(br.columns)} cols')

    print('\n[3/9] Running step3 price enrichment for BR...')
    try:
        prices = run_step3_for_br(br)
    except Exception as e:
        print(f'  WARN: step3 failed ({e}) — continuing without price data')
        prices = pd.DataFrame()

    print('\n[4/9] Merging snapshots + prices + macro...')
    df = merge_br_data(br, prices)
    ...
    print('\n[9/9] Aligning BR rows to target schema...')
    for col in target_cols:
        if col not in df.columns:
            df[col] = np.nan
    br_aligned = df[target_cols].copy()
    print(f'  BR aligned: {len(br_aligned):,} rows × {len(br_aligned.columns)} cols')

    annual_br = (br_aligned['period_type'] == 'annual').sum() if 'period_type' in br_aligned.columns else 0
    print(f'\n  BR rows summary:')
    print(f'    Total rows:  {len(br_aligned):,}')
    print(f'    Annual rows: {annual_br:,}')
    if 'entry_price' in br_aligned.columns:
        print(f'    Rows with entry_price: {br_aligned["entry_price"].notna().sum():,}')
    if 'beneish_m_score' in br_aligned.columns:
        print(f'    Rows with beneish_m_score: {br_aligned["beneish_m_score"].notna().sum():,}')
    ...
    print('\n  Concatenating...')
    combined = pd.concat([existing, br_aligned], ignore_index=True)
    ...
    print(f'\n  Saved: {CLEAN}')
    print('Phase A — BR complete.')


def main() -> None:
    parser = argparse.ArgumentParser(description='Integrate BR market data (Phase A)')
