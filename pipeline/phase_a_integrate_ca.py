"""
Phase A — Integrate Canada (CA) market data into historical_dataset_clean.parquet

CA snapshots (data/snapshots_ca.parquet) are built via SEDAR+ iXBRL parsing with
yfinance as fallback. Data covers TSX/TSXV listed companies.

This script:
  1. Loads data/snapshots_ca.parquet and standardises column names
  2. Runs step3 price enrichment for CA tickers -> data/prices_ca.parquet
  3. Merges CA snapshots + prices + macro
...
CA_SNAPSHOTS = DATA / 'snapshots_ca.parquet'
CA_PRICES    = DATA / 'prices_ca.parquet'
CA_STD       = DATA / 'snapshots_ca_std.parquet'
MACRO        = DATA / 'macro_ca.parquet'
...
def standardise_ca_snapshots(df: pd.DataFrame) -> pd.DataFrame:
    """Add missing columns and aliases so step5 functions can run on CA data."""
    df = df.copy()
...
def run_step3_for_ca(ca_std: pd.DataFrame) -> pd.DataFrame:
    ca_std.to_parquet(CA_STD, index=False)
    print(f'  Saved standardised CA snapshots: {CA_STD} ({len(ca_std):,} rows)')

    import pipeline.step3_enrich_prices as step3
    print('  Running step3 price enrichment for CA tickers...')
    step3.run(snapshots_path=str(CA_STD), out_path=str(CA_PRICES))

    if not CA_PRICES.exists():
        print('  WARN: step3 produced no output — prices_ca.parquet missing')
        return pd.DataFrame()

    prices = pd.read_parquet(CA_PRICES)
    print(f'  Loaded CA prices: {len(prices):,} rows × {len(prices.columns)} cols')
    return prices


def merge_ca_data(ca: pd.DataFrame, prices: pd.DataFrame) -> pd.DataFrame:
    macro_path = MACRO if MACRO.exists() else MACRO_FALLBACK

    if prices.empty:
        print('  WARN: No price data — CA rows will have NaN price/return columns')
        merged = ca.copy()
    else:
        price_cols = set(prices.columns)
        avail_keys = [k for k in MERGE_KEYS if k in ca.columns and k in price_cols]
        print(f'  Merging on keys: {avail_keys}')
        snap_cols = set(ca.columns)
        price_only_cols = [c for c in prices.columns if c not in snap_cols or c in avail_keys]
        merged = ca.merge(prices[price_only_cols], on=avail_keys, how='left', suffixes=('', '_price'))
...
def run(dry_run: bool = False) -> None:
    print('Phase A — Integrate Canada (CA) market data')
    print('=' * 60)

    if not CA_SNAPSHOTS.exists():
        print(f'ERROR: {CA_SNAPSHOTS} not found — run run_pipeline_ca.py build first')
        sys.exit(1)
    if not CLEAN.exists():
    ...
    if 'market' in existing.columns:
        ca_existing = (existing['market'] == 'CA').sum()
        if ca_existing > 0:
            print(f'  NOTE: {ca_existing:,} CA rows already present — will be replaced')
            existing = existing[existing['market'] != 'CA'].copy()

    print('\n[2/9] Loading and standardising CA snapshots...')
    ca_raw = pd.read_parquet(CA_SNAPSHOTS)
    print(f'  CA raw: {len(ca_raw):,} rows × {len(ca_raw.columns)} cols')
    print(f'  Annual rows: {(ca_raw.get("period_type", pd.Series(dtype=str)) == "annual").sum():,}')

    ca = standardise_ca_snapshots(ca_raw)
    print(f'  After standardisation: {len(ca.columns)} cols')

    print('\n[3/9] Running step3 price enrichment for CA...')
    try:
        prices = run_step3_for_ca(ca)
    except Exception as e:
        print(f'  WARN: step3 failed ({e}) — continuing without price data')
        prices = pd.DataFrame()

    print('\n[4/9] Merging snapshots + prices + macro...')
    df = merge_ca_data(ca, prices)
    ...
    print('\n[9/9] Aligning CA rows to target schema...')
    for col in target_cols:
        if col not in df.columns:
            df[col] = np.nan
    ca_aligned = df[target_cols].copy()
    print(f'  CA aligned: {len(ca_aligned):,} rows × {len(ca_aligned.columns)} cols')

    annual_ca = (ca_aligned['period_type'] == 'annual').sum() if 'period_type' in ca_aligned.columns else 0
    print(f'\n  CA rows summary:')
    print(f'    Total rows:  {len(ca_aligned):,}')
    print(f'    Annual rows: {annual_ca:,}')
    if 'entry_price' in ca_aligned.columns:
        print(f'    Rows with entry_price: {ca_aligned["entry_price"].notna().sum():,}')
    if 'beneish_m_score' in ca_aligned.columns:
        print(f'    Rows with beneish_m_score: {ca_aligned["beneish_m_score"].notna().sum():,}')
    ...
    print('\n  Concatenating...')
    combined = pd.concat([existing, ca_aligned], ignore_index=True)
    ...
    print(f'\n  Saved: {CLEAN}')
    print('Phase A — CA complete.')


def main() -> None:
    parser = argparse.ArgumentParser(description='Integrate CA market data (Phase A)')
