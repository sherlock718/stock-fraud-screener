...
Run this once in a terminal and leave it. It will:
  ...
  2. Run scripts/merge_snapshots.py --activate --backup
  3. Run python3 scripts/run_pipeline.py build --step 4  (features + clean on full dataset)
...
run(['python3', 'scripts/merge_snapshots.py', '--activate', '--backup'])

print('\n─── Step 2: Re-running features + clean on merged dataset ───')
run(['python3', 'scripts/run_pipeline.py', 'build', '--step', '4'])
