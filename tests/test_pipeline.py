...
# ─── app_v2.py score_companies ────────────────────────────────────────────────

class TestScoreCompanies:
    def _make_mock_model(self, n_features: int = 5):
        """Return a minimal mock that implements predict_proba."""
        class _MockClf:
            def predict_proba(self, X):
                return np.column_stack([np.full(len(X), 0.4), np.full(len(X), 0.6)])
        return _MockClf()

    def test_uses_train_medians(self):
        import sys, os
        sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..'))
        # Import just the function to avoid triggering Streamlit
        import importlib.util, types
        spec = importlib.util.spec_from_file_location(
            'app_v2',
            os.path.join(os.path.dirname(__file__), '..', 'app_v2.py')
        )
        # We only need score_companies — patch streamlit before import
        ...
        score_companies = mod.score_companies

        feats = ['gross_margin', 'roe', 'pe_ratio', 'current_ratio', 'debt_to_equity']
        train_medians = {f: 0.5 for f in feats}

        df = _make_annual_df(n_tickers=5, n_years=2)
        # Introduce NaN to verify median fill path
        df.loc[0, 'gross_margin'] = np.nan

        models = {'1y': self._make_mock_model()}
        meta   = {'1y': {'features': feats, 'train_medians': train_medians}}

        result = score_companies(df, models, meta, horizon='1y')
        assert 'ml_score' in result.columns
        assert result['ml_score'].notna().all()

    def test_missing_horizon_returns_nan(self):
        import sys, os
        import importlib.util, types
        st_mock = sys.modules.get('streamlit', types.ModuleType('streamlit'))
        st_mock.cache_data    = lambda **kw: (lambda f: f)  # type: ignore[attr-defined]
        st_mock.cache_resource = lambda **kw: (lambda f: f)  # type: ignore[attr-defined]
        ...
        df = _make_annual_df(n_tickers=3, n_years=2)
        result = mod.score_companies(df, {}, {}, horizon='1y')
        assert (result['ml_score'].isna()).all()
